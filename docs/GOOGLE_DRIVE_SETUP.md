...
REACT_APP_GOOGLE_DRIVE_REDIRECT_URI=http://localhost:8888/
```
...
5. Frontend exchanges code for access token via `/auth-google-callback` endpoint
...
- Check that `REACT_APP_GOOGLE_DRIVE_REDIRECT_URI=http://localhost:8888/` exactly matches Google Cloud Console settings
...
- Local: must be `http://localhost:8888/`
...
- **OAuth handler**: `netlify/functions/auth-google-callback.ts`
