...
const triggerCron = () => {
  const req = http.request(
    {
      hostname: "localhost",
      port: 8888,
      path: "/.netlify/functions/notification-cron",
      method: "POST",
    },
    (res) => {
      let data = "";
      ...
      res.on("end", () => {
        console.log(
          `[Cron Simulator] Odpytano cron o godzinie: ${new Date().toLocaleTimeString()} - Odpowiedź: ${res.statusCode}`,
        );
      });
    },
  );
  ...
  req.on("error", (e) => {
    console.error(
      `[Cron Simulator] Porażka - nie mogliśmy uderzyć w Cron: ${e.message} (czy na pewno uruchomiłeś npm run dev na porcie 8888?)`,
    );
  });
